...
# Place the values in the environment variables CLIENT_ID and CLIENT_SECRET.
...
```powershell
# First get JSON Web Token
# Ensure the environment variables CLIENT_ID, CLIENT_SECRET and JWT_TOKEN are set in your session or system.
...
```cmd
:: First get JSON Web Token
:: Ensure the environment variables CLIENT_ID, CLIENT_SECRET and JWT_TOKEN are set before running.
curl -v "https://api.groupdocs.cloud/connect/token" ^
  -X POST ^
  -d "grant_type=client_credentials&client_id=%CLIENT_ID%&client_secret=%CLIENT_SECRET%" ^
  -H "Content-Type: application/x-www-form-urlencoded" ^
...
curl -v "https://api.groupdocs.cloud/v2.0/viewer/view" ^
  ...
  -H "Accept: application/json" ^
  -H "Authorization: Bearer %JWT_TOKEN%" ^
  -d "{ \"FileInfo\": { \"FilePath\": \"SampleFiles/sample.docx\" }, \"ViewFormat\": \"JPG\", \"RenderOptions\": { \"JpegQuality\": 50 } }"
```
{{< /tab >}} {{< tab "Response" >}}
```json
{
