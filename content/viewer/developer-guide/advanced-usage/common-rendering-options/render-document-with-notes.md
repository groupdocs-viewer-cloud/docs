...
```powershell
# First get JSON Web Token
# Please get your Client Id and Client Secret from https://dashboard.groupdocs.cloud/applications. Set them in environment variables $env:CLIENT_ID and $env:CLIENT_SECRET.
...
curl.exe -v "https://api.groupdocs.cloud/v2.0/viewer/view" `
  ...
  -d "{ 'FileInfo': { 'FilePath': 'SampleFiles/with_notes.pptx' }, 'ViewFormat': 'HTML', 'RenderOptions': { 'RenderNotes': true } }"
```
{{< /tab >}}
...
```cmd
:: First get JSON Web Token
:: Please get your Client Id and Client Secret from https://dashboard.groupdocs.cloud/applications. Set them in environment variables %CLIENT_ID% and %CLIENT_SECRET%.
curl -v "https://api.groupdocs.cloud/connect/token" ^
  -X POST ^
  -d "grant_type=client_credentials&client_id=%CLIENT_ID%&client_secret=%CLIENT_SECRET%" ^
  -H "Content-Type: application/x-www-form-urlencoded" ^
