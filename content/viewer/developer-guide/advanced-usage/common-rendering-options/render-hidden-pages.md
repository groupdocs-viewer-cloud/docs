...
# They are supplied via environment variables CLIENT_ID and CLIENT_SECRET.
...
# Get document information using the obtained JWT token.
...
# First get JSON Web Token
# Retrieve your Client Id and Client Secret from https://dashboard.groupdocs.cloud/applications.
# They are supplied via environment variables $env:CLIENT_ID and $env:CLIENT_SECRET.
...
curl -v "https://api.groupdocs.cloud/v2.0/viewer/view" ^
  ...
  -H "Accept: application/json" ^
  -H "Authorization: Bearer %JWT_TOKEN%" ^
  -d "{\"FileInfo\":{\"FilePath\":\"SampleFiles/with_hidden_page.pptx\"},\"ViewFormat\":\"HTML\",\"RenderOptions\":{\"RenderHiddenPages\":true}}"
```
{{< /tab >}} {{< tab "Response" >}}
```json
{
