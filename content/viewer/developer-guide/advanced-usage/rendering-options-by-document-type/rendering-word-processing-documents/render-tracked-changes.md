...
# Place them in the $CLIENT_ID and $CLIENT_SECRET environment variables.
...
```cmd
:: First get JSON Web Token
:: Ensure CLIENT_ID, CLIENT_SECRET and JWT_TOKEN are defined as environment variables.
curl -v "https://api.groupdocs.cloud/connect/token" ^
 -X POST ^
 -d "grant_type=client_credentials&client_id=%CLIENT_ID%&client_secret=%CLIENT_SECRET%" ^
 -H "Content-Type: application/x-www-form-urlencoded" ^
...
curl -v "https://api.groupdocs.cloud/v2.0/viewer/view" ^
 ...
 -H "Accept: application/json" ^
 -H "Authorization: Bearer %JWT_TOKEN%" ^
 -d "{\"FileInfo\":{\"FilePath\":\"SampleFiles/with_tracked_changes.docx\"},\"ViewFormat\":\"HTML\",\"RenderOptions\":{\"WordProcessingOptions\":{\"RenderTrackedChanges\":true}}}"
```
{{< /tab >}} {{< tab "Response" >}}
```json
{
  "pages": [
    {
      "number": 1,
      "resources": null,
      "path": "viewer/with_tracked_changes_docx/with_tracked_changes_page_1.html",
      "downloadUrl": "https://api.groupdocs.cloud/v2.0/viewer/storage/file/viewer/with_tracked_changes_docx/with_tracked_changes_page_1.html"
    }
  ],
  "attachments": [],
  "file": null
