...
```bash
# Get JSON Web Token
# Client credentials are taken from environment variables CLIENT_ID and CLIENT_SECRET.
...
# Get document information (Viewer API)
...
curl -v "https://api.groupdocs.cloud/v2.0/viewer/view" ^
  ...
  -H "Accept: application/json" ^
  -H "Authorization: Bearer %JWT_TOKEN%" ^
  -d "{\"FileInfo\":{\"FilePath\":\"SampleFiles/sample.ost\"},\"ViewFormat\":\"HTML\",\"RenderOptions\":{\"OutlookOptions\":{\"Folder\":\"Inbox\"}}}"
```
{{< /tab >}} {{< tab "Response" >}}
```json
{
